...
// Tạo URL để admin đăng nhập YouTube, có gắn state = userId
...
async function getAuthorizedClientForUser(userId) {
  ...
  const row = rows[0];
  if (!row) {
    throw new Error("No google token stored for this user");
  }
...
export async function syncChannelDailyStats(
  userId,
  channelId,
  startDate,
  endDate
  ...
  for (const row of rows) {
    const [dateStr, views, revenue] = row;
